...
export default function OrderWizard({ onComplete }: OrderWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    // Validate quantities
    const invalidItem = orderItems.find(item => item.quantity > item.available_quantity);
    if (invalidItem) {
      toast({
        title: "Error",
    ...
    const totalCharges = Number(orderData.handling_charges) + Number(orderData.delivery_charges);

    // Generate order number
    const orderNumber = `OUT-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 10000)).padStart(5, '0')}`;
  ...
  return (
    <Tabs value={step} onValueChange={setStep} className="w-full">
      <TabsList className="grid w-full grid-cols-3">
        <TabsTrigger value="1">Order Details</TabsTrigger>
      ...
      <TabsContent value="1" className="space-y-4 mt-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Select value={orderData.customer_id} onValueChange={(value) => setOrderData({...orderData, customer_id: value})}>
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select value={orderData.warehouse_id} onValueChange={(value) => setOrderData({...orderData, warehouse_id: value})}>
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select value={orderData.order_type} onValueChange={(value) => setOrderData({...orderData, order_type: value})}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setOrderData({...orderData, requested_date: e.target.value})}
            />
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOrderData({...orderData, delivery_contact_name: e.target.value})}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOrderData({...orderData, delivery_contact_phone: e.target.value})}
                  />
                </div>
                <div className="space-y-2 col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOrderData({...orderData, delivery_address_line1: e.target.value})}
                  />
                </div>
                <div className="space-y-2">
                  <Label>City</Label>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOrderData({...orderData, delivery_postal_code: e.target.value})}
                  />
                </div>
              </div>
            </CardContent>
        ...
        <div className="flex justify-end">
          <Button onClick={() => setStep("2")} disabled={!orderData.customer_id || !orderData.warehouse_id}>
            Next: Add Items
          </Button>
        </div>
      </TabsContent>
                      ...
                      <SelectContent>
                        {inventory.map(inv => (
                          <SelectItem key={inv.id} value={inv.id}>
                            {inv.item_name} ({inv.item_code}) - Available: {inv.quantity}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setOrderData({...orderData, handling_charges: parseFloat(e.target.value) || 0})}
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setOrderData({...orderData, delivery_charges: parseFloat(e.target.value) || 0})}
                />
              </div>
            </div>
